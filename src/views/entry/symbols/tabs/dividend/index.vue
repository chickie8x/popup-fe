<template>
  <div class="border-2 border-gray-200 rounded-lg min-h-full flex">
    ...
    <div class="w-2/3 h-full"> <!-- table wrapper -->
              ...
              <div class="shadow-sm ring-1 ring-black ring-opacity-5">
                <table class="min-w-full border-separate" style="border-spacing: 0">
                  <thead class="">
                    <tr>
                      <th
                        v-for="header, idx in headers" :key="idx"
                        scope="col"
                  ...
                  <tbody class="bg-white">
                    <tr 
                      v-for="(event, evIdx) in events" :key="evIdx"
                      :class="[evIdx%2===0?' bg-gray-100':'', 'cursor-pointer hover:bg-gray-200']"
                     >
                      <td
                        :class="[, 'whitespace-nowrap py-4 pl-4 pr-3 text-sm text-gray-900 sm:pl-6 lg:pl-8']"
                      >{{ $filters.formatDate(event.recordDate) }}</td>
                      ...
                      >{{ event.title }}</td>
                       ...
                       <td
                        :class="[, 'whitespace-nowrap py-4 pl-4 pr-3 text-sm text-gray-900 sm:pl-6 lg:pl-8']"
                      >{{ $filters.formatDate(event.executionDate) }}</td>
    ...
    <div class="w-1/3 h-full bg-[#d1e5f1]"> <!-- charts wrapper -->
      <div class="">
